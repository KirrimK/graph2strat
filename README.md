...
```python
# Generated by graph2strat v1.5.0 by KirrimK@ENAC
...
`loop:function_name`: the function `function_name` will be called repeatedly while in the state

Combine all those by separating them with a semicolon:
...
```dot
NameOfInitState -> NameOfState [label="type:function_name;..."] // declares a transition from NameOfInitState to NameOfState

{StateA StateB StateC ...} -> NameOfState [label="type:function_name;..."] // declares a transition from multiple states to NameOfState

```

Here are examples of what to specify in the label field:

`guard:function_name`: the function `function_name` will be checked to see if transition should be executed (expected output is a boolean: True to activate transition, False to ignore it)
`action:function_name`: the function `function_name` will be called when activating the transition

Combine all those by separating them with a semicolon:
`guard:function_name;action:function_name`
